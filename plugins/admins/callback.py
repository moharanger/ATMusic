...
from config import (BANNED_USERS, SOUNCLOUD_IMG_URL, STREAM_IMG_URL,
                    ...
                    adminlist)
from ATMusic import YouTube, app
from ATMusic.core.call import AT
from ATMusic.misc import SUDOERS, db
from ATMusic.utils.database import (is_active_chat, is_music_playing, is_muted,
                                    is_nonadmin_chat, music_off, music_on,
                                    mute_off, mute_on, set_loop)
from ATMusic.utils.decorators.language import languageCB
from ATMusic.utils.formatters import seconds_to_min
from ATMusic.utils.inline.play import stream_markup, telegram_markup
from ATMusic.utils.stream.autoclear import auto_clean
from ATMusic.utils.thumbnails import gen_thumb
    ...
    if command == "Pause":
        ...
        try:
            ...
            if not check:
                ...
                try:
                    return await AT.stop_stream(chat_id)
                except:
                    return
        except:
            try:
        ...
        if "live_" in queued:
            ...
            try:
                await AT.skip_stream(chat_id, link, video=status)
            except Exception:
            ...
            try:
                await AT.skip_stream(chat_id, file_path, video=status)
            except Exception:
                return await mystic.edit_text(_["call_7"])
            ...
            try:
                await AT.skip_stream(chat_id, videoid, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(_["call_7"])
            button = telegram_markup(_, chat_id)
            run = await CallbackQuery.message.reply_photo(
            ...
            try:
                await AT.skip_stream(chat_id, queued, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(_["call_7"])
            if videoid == "telegram":
                button = telegram_markup(_, chat_id)
